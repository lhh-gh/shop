...
│  第三道: TTL 自然过期                                               │
│  │ 所有缓存都有过期时间                                             │
│  │ 保证：即使以上全失败，最多 TTL 时间后自动恢复一致                  │
│  │                                                                 │
│  第四道: 定时对账                                                   │
│  │ 每小时抽样检查 MySQL 与 Redis 数据是否一致                       │
│  │ 保证：发现并修复长期不一致                                       │
│  │                                                                 │
│  第五道: 全量同步（人工触发）                                       │
│  │ 出现大规模不一致时，清空 Redis 重建缓存                          │
│  │ 命令：php artisan cache:rebuild-products                        │
│  │ 保证：最终兜底手段                                               │
│  │                                                                 │
│  结论：数据以 MySQL 为准，Redis 只是加速层                           │
│       短暂不一致可接受（商品价格显示晚几秒更新 ≠ 支付错误金额）      │
│       支付金额永远从 MySQL 实时读取，不走缓存                        │
│                                                                    │
└────────────────────────────────────────────────────────────────────┘
```

> **核心原则：关键数据（订单、支付、库存扣减）不走缓存，直接 MySQL。缓存只用于读多写少的展示型数据（商品详情、分类列表、用户信息）。这样即使缓存不一致，最多是展示延迟，不会造成资金或库存错误。**
