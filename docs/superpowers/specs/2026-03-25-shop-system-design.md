...
- **数据库**: MySQL（核心业务数据）
- **搜索引擎**: Elasticsearch 8.x（商品全文搜索、搜索建议、聚合筛选）→ 详见 [ES+MongoDB 设计文档](./2026-03-25-es-mongodb-design.md)
- **文档库**: MongoDB 7.x（浏览记录、搜索日志、操作审计）→ 详见 [ES+MongoDB 设计文档](./2026-03-25-es-mongodb-design.md)
...
GET    /api/v1/user/browse-history          我的浏览记录（MongoDB）
DELETE /api/v1/user/browse-history          清空浏览记录
GET    /api/v1/user/search-history          我的搜索历史（MongoDB）
...
│   │   ├── ProductSearchRepositoryInterface   商品搜索（ES）
│   │   ├── ProductViewRepositoryInterface     浏览记录（MongoDB）
│   │   ├── SearchLogRepositoryInterface       搜索日志（MongoDB）
│   │   ├── OperationLogRepositoryInterface    审计日志（MongoDB）
...
│   │   └── ElasticProductSearchRepository   商品搜索
│   ├── Mongo/                       MongoDB 实现
...
│   │   └── ProductSearchService         商品搜索（ES 查询 + MySQL 降级）
│   │   └── ProductViewService           商品浏览记录（MongoDB）
│   ├── Cart/CartService
│   ├── Promotion/                   营销模块
...
│   ├── Search/                     搜索相关
│   │   ├── ElasticProductIndexer       ES 索引管理（增删改/批量）
│   │   ├── ElasticQueryBuilder         ES 查询 DSL 构建器
│   │   └── SearchLogService            搜索日志（MongoDB）
│   └── Audit/
│       └── OperationLogService         操作审计日志（MongoDB）
│
├── Enums/                           PHP 8.2+ 枚举
│   ├── OrderStatus
...
**缓存失效策略：** 写操作后主动删除对应缓存 key，下次读取时自动回填。采用延迟双删 + TTL 兜底保证 MySQL-Redis 一致性，详见 [高并发与数据一致性设计文档](./2026-03-25-concurrency-consistency-design.md)。
...
> 完整的高并发解决方案（含秒杀 Redis 预扣库存、优惠券超领防护、支付回调幂等、订单防重复、缓存穿透/雪崩/击穿、MySQL-Redis 数据一致性）详见 [高并发与数据一致性设计文档](./2026-03-25-concurrency-consistency-design.md)。
...
4. **Elasticsearch 集成** — 索引创建/数据同步/商品搜索/搜索建议（详见 ES+MongoDB 设计文档）
5. **营销模块** — 促销活动/优惠券/责任链优惠计算管道
6. **购物车** — 加购/合并/选中/失效检测
7. **订单与支付** — 下单预览/下单（含优惠计算）/支付网关/回调/状态机
8. **MongoDB 集成** — 浏览记录/搜索日志/操作审计（详见 ES+MongoDB 设计文档，可与 5-7 并行）
9. **物流** — 依赖订单
10. **售后** — 依赖订单和支付
8. **售后** — 依赖订单（退款+优惠券退回）
